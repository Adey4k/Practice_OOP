...
public class MainTest {
    ...
    /**
     * Перевіряє паралельний аналіз кількості одиниць у двійковому представленні.
     */
    @Test
    public void testParallelAnalysis() {
        calc.init(90.0, 90.0, 90.0, 90.0);
        calc.init(180.0, 180.0, 180.0, 180.0);
        calc.init(270.0, 270.0, 270.0, 270.0);
        
        // Запускаємо аналіз та перевіряємо, чи не викликає помилок
        try {
            calc.analyzeParallel();
        } catch (Exception e) {
            fail("Parallel analysis failed: " + e.getMessage());
        }
    }
}